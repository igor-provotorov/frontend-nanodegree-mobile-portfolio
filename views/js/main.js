...
var selectRandom = function (pizzaIngredients) {
  var random = pizzaIngredients[Math.floor((Math.random() * pizzaIngredients.length))];
  return random;
};
...
var makeRandomPizza = function () {
  ...
  for (var i = 0; i < numberOfMeats; i++) {
    pizza = pizza + ingredientItemizer(selectRandom(pizzaIngredients.meats));
  }

  for (var j = 0; j < numberOfNonMeats; j++) {
    pizza = pizza + ingredientItemizer(selectRandom(pizzaIngredients.nonMeats));
  }

  for (var k = 0; k < numberOfCheeses; k++) {
    pizza = pizza + ingredientItemizer(selectRandom(pizzaIngredients.cheeses));
  }

  pizza = pizza + ingredientItemizer(selectRandom(pizzaIngredients.sauces));
  pizza = pizza + ingredientItemizer(selectRandom(pizzaIngredients.crusts));
